...
function App() {
  ...
  const loadFixture = useCallback(async (fixture) => {
    ...
    try {
      // Try live backend first, fall back to embedded static fixtures
      let matterData, graphData, cons
      try {
        const data = await fetch(`/demo?fixture=${fixture}`).then(r => {
        ...
        matterData = await mRes.json()
        graphData = await gRes.json()
        cons = await cRes.json()
      } catch (_apiErr) {
        // Static mode: use embedded fixture data
        const staticData = typeof STATIC_FIXTURES !== 'undefined' && STATIC_FIXTURES[fixture]
        if (!staticData) throw new Error('No backend available and no static data found')
        setMatterId('static-' + fixture)
        matterData = staticData.matter
        graphData = staticData.graph
        cons = staticData.contradictions
      }
  ...
  const downloadCDD = useCallback(async () => {
    if (!matterId) return
    if (matterId.startsWith('static-')) {
      alert('CDD report generation requires the backend server. Run the app locally to use this feature.')
      return
    }
